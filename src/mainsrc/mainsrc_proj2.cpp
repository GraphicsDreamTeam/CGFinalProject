...
void Setup(void)
{
    ...
    Camera* camera = new Camera(STVector3(0, 0, -1), STVector3(0, 0, 0), STVector3(0.0f, 1.0f, -1.0f));
    camera->SetFov(100);

    // Testing for setting the camera to look at the center as we move it around
    // STVector3 newLookAt = -camera->GetPosition();
    // newLookAt.Normalize();
    ...
    pScene->AddLight(Light(STVector3(10, 0, -35), RGBR_f(255, 0, 0, 255), 40, "Light1"));
    pScene->AddLight(Light(STVector3(-5, 8.66, -35), RGBR_f(0, 255, 0, 255), 40, "Light2"));
    pScene->AddLight(Light(STVector3(-5, -8.66, -35), RGBR_f(0, 0, 255, 255), 40, "Light3"));
    // pScene->AddLight(Light(STVector3(-5, -8.66, -30), RGBR_f(255, 255, 255, 255), 30, "Light3"));

    // X, Y, Z.
    // Z is stationary at negative 1
    // X is as you'd expect, Y is inverse (of a standard graph)

    // Sphere* sphere1 = new Sphere(STVector3(0, 0, 10), 2);
    // pScene->AddSurface(sphere1);

    // Sphere* sphere2 = new Sphere(STVector3(0.866, 0.5, -39), 0.25, RGBR_f(255, 255, 255, 255));
    // pScene->AddSurface(sphere2);

    Sphere* sphere3 = new Sphere(STVector3(0, 0, -40), 0.75, RGBR_f(255, 255, 255, 255));
    pScene->AddSurface(sphere3);

    // Sphere* sphere2 = new Sphere(STVector3(-0.65, 0, -40), 0.5, RGBR_f(255, 255, 255, 255));
    // pScene->AddSurface(sphere2);

    // Sphere* sphere3 = new Sphere(STVector3(0.65, 0, -40), 0.5, RGBR_f(255, 255, 255, 255));
    // pScene->AddSurface(sphere3);
